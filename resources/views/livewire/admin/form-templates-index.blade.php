...
<div class="py-8">
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        {{-- Flash messages --}}
        @if (session('success'))
            <div class="p-3 rounded bg-green-100 text-black px-4 py-3 shadow-sm">
                {{ session('success') }}
            </div>
        @endif

        @if (session('error'))
            <div class="p-3 rounded bg-red-100 text-black px-4 py-3 shadow-sm">
                {{ session('error') }}
            </div>
        @endif
        ...
        <div class="flex gap-3 items-center">
            ...
            <select
                id="approvalStatus"
                wire:model.live="approvalStatus"
            ...
            <tr class="text-left">
                ...
                @forelse ($templates as $t)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <a
                                href="{{ route('livewire.admin.show', $t) }}"
                                class="text-blue-500 hover:text-indigo-800 hover:underline"
                            >
                                {{ $t->title }}
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    @if($t->approval_status === 'approved') bg-green-100
                                    @elseif($t->approval_status === 'pending') bg-amber-200
                                    @elseif($t->approval_status === 'rejected') bg-red-100
